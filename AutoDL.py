...
class Downloader:
	...
	def download(self, filelist):
		...
		for x in filelist:
			...
			while not download_complete and self.mirror_count < self.mirror_total:
				self.caller.report_file_info(x['filename'], x['urllist'][self.mirror_count], self.mirror_count+1,self.mirror_total )
				self.caller.refresh()
...
class AutoDL(object):
	...
	def init(self):
		...
		if self.error_logger.flag():
			...
			self.widgets['message_title_label'].set_text(self.settings['messagelist'][0]['title'])
	...
	def on_unclean_exit_event(self, *args):
		...
		try:
			os.remove(self.downloader.remaining_files[0]['path'] + '/' +
	...
	def report_no_next_mirror(self):
		self.widgets['misc_download_label'].set_text("No next mirror available, press 'Stop download' to quit now and retry later." )
		self.refresh()
	#----------------------------------------------------------------------

	def report_broken_link(self, url):

		self.widgets['misc_download_label'].set_text("The link " + url + " appears to be broken. Trying next mirror.")
		self.refresh()
	#----------------------------------------------------------------------
	...
	def report_file_info(self, filename, url, mirror_count, mirror_total):
		...
		self.widgets['current_download_label'].set_text("Filename: " + filename + "\nLocation: " + url)
		self.widgets['misc_download_label'].set_text('')
		self.widgets['mirror_label'].set_text('Mirror ' + text_count + ' / ' + text_total)
...
#Open ini file and put it's content into a string
